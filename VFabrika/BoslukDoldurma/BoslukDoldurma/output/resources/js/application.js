    ...
    function FillInTheBlanks() {
        ...
        this.initialize = function () {
            ...
            for (var i = 0; i < answers.length; i++) {
                ...
                htmlText = htmlText.replace("[" + answerValue + "]", "<div id='" + answerDivName + "' class='answer'>&nbsp;</div>");
            }
            ...
            $answerDropDown = $("#app-text-answer-dropdown");
            for (var i = 0; i < answers.length; i++) {
                $answerDropDown.append("<div id='answer-dropdown-item" + i + "' class='app-text-answer-dropdown-item'>" + answers[i].value.toLowerCase() + "</div>");
                var $answerDropDownItem = $("#answer-dropdown-item" + i);
                $answerDropDownItem.data("value", answers[i].value.toLowerCase());
                $answerDropDownItem.on("click", self.onAnswerDropDownItemClicked);
            }
        ...
        this.onAnswerClicked = function (e) {
            ...
            if ($answerDiv.attr("disabled") == null) {
                self.startTimer();
        ...
        this.onCheckButtonClicked = function () {
            ...
            for (var i = 0; i < answers.length; i++) {
                ...
                if ($answerDiv.length > 0) {
                    if ($answerDiv.html().toLowerCase() != answers[i].value.toLowerCase()) {
                        $answerDiv.css("background-color", "#FF4500");
                        allOk = false;
                    } else {
                        $answerDiv.css("background-color", "#32CD32");
                        $answerDiv.attr("disabled", true);
